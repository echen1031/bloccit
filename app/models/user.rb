class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*'). #Select all attributes of the user
        ...
        select('COUNT(DISTINCT comments.id) + COUNT(DISTINCT posts.id) AS rank'). #Add the comment to the post count and label the sum as "rank"
        ...
        group('users.id'). #Instructs the database to group the results so that each suer will be returned in a distinct row
